...
module.exports = function(socket, connections) {
  ...
  socket.on('connect', function(data) {
    ...
    connection[data.server].addSocket(socket);
    ...
    socket.on('part_pm', function(name, server) {
      if(connection[server].clients.chans[name.toLowerCase()] !== undefined) {
        delete connection[server].clients.chans[name.toLowerCase()];
      }
    });
    ...
    socket.on('part', function(name, server) {
      ...
      connection[data.server].client.part(name);
      if(current_user){
    ...
    socket.on('disconnect', function() {
      if(!current_user){
        ...
        for( srv in connection ) {
          connection[srv].removeSocket(socket);
          connection[srv].clearUnreads();
        }
      }
    });
    ...
    socket.on('getOldMessages', function(data) {
      if (current_user) {
        var query = Message.find({channel: data.channelName.toLowerCase(),
          server: connection[data.server].server.toLowerCase(), linkedto: current_user.username});
